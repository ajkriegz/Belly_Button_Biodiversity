...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var samplesObject = data.samples;

    // 4. Create a variable that filters the samples for the object with the desired sample number.
    var sampleArray = samplesObject.filter(obj => obj.id == sample);

    // 3.1. Create a variable that filters the metadata array for the object with the desired sample number.
    var metaMetadata = data.metadata;
    var metaArray = metaMetadata.filter(sampleObj => sampleObj.id == sample);
    
    //  5. Create a variable that holds the first sample in the array.
    var result2 = sampleArray[0];
